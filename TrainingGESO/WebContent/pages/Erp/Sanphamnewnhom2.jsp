<%@page import="geso.erp.beans.sanpham_nhom2.ISanpham"%>
...
<% 
	...
	ISanpham nccBean = (ISanpham) session.getAttribute("spnew"); 
	String userId = (String) session.getAttribute("userId");
	String userTen = (String) session.getAttribute("userTen");
  
	ResultSet rsdvt=nccBean.getRsDvt();
...
<BODY leftmargin="0" bottommargin="0" topmargin="0" rightmargin="0">
	<form name="dmhForm" method="post" action="../../SanphamNhom2NewSvl">
		<input type="hidden" name="userId" value='<%=userId%>'> <input
			type="hidden" name="action" value="1" /> <input type="hidden"
			name="id" value='<%=nccBean.getId()%>'>
		<div id="main" style="width: 100%; padding-left: 2px">
			<div align="left" id="header" style="width: 100%; float: none">
				<div style="width: 60%; padding: 5px; float: left"
					class="tbnavigation">Thong tin sanpham</div>
			...
			<div align="left" style="width: 100%; float: none; clear: left">
				<fieldset>
					...
					<textarea name="dataerror" id="Msg" rows="1" readonly="readonly"
						style="width: 100%"><%=nccBean.getmsg()%></textarea>
				</fieldset>
			</div>
			<div align="left" style="width: 100%; float: none; clear: left">
				<fieldset>
						...
						<TABLE width="100%" cellpadding="4" cellspacing="0">
							...
							<TR>
								...
								<TD class="plainlabel"><input type="text" name="ma"
									value="<%=nccBean.getMa()%>"></TD>

							</TR>
							<TR>
								<TD class="plainlabel">Tên</TD>
								<TD class="plainlabel"><input type="text" name="ten"
									value="<%=nccBean.getTen()%>"></TD>

							</TR>
							<TR>
								<TD class="plainlabel">Donvi</TD>
								<TD class="plainlabel"><select name="donvi"
									style="width: 500px">
										<option></option>
										<%if(rsdvt!=null) {
										while(rsdvt.next()) {
											if(rsdvt.getString("pk_seq").equals(nccBean.getDvt())){
												%>
										<option value="<%=rsdvt.getString("pk_seq")%>"
											selected="selected"><%=rsdvt.getString("ma") %>
										</option>
										<%
											}else{
												 %>
										<option value="<%=rsdvt.getString("pk_seq")%>"><%=rsdvt.getString("ma") %>
										</option>
										<%
											}
										}
							...
							<TR>
								...
								<TD class="plainlabel">
									<% if(nccBean.getTrangthai().equals("1")) {%> <input
									type="checkbox" name="tinhtrang" checked="checked" value="1">
									<%}else {%> <input type="checkbox" name="tinhtrang" value="1">
